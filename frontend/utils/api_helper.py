# # # import requests
# # # import streamlit as st

# # # BACKEND_URL = "http://127.0.0.1:8000/api/v1"


# # # def signup(username, email, password):
# # #     response = requests.post(
# # #         f"{BACKEND_URL}/auth/signup",
# # #         json={"username": username, "email": email, "password": password},
# # #     )
# # #     return response


# # # def login(email, password):
# # #     response = requests.post(
# # #         f"{BACKEND_URL}/auth/login",
# # #         data={"username": email, "password": password},
# # #     )
# # #     return response


# # # def predict(image, token):
# # #     headers = {"Authorization": f"Bearer {token}"}
# # #     files = {"file": image}
# # #     response = requests.post(
# # #         f"{BACKEND_URL}/predict", headers=headers, files=files
# # #     )
# # #     return response


# # # # def get_user_profile(token: str):
# # # #     """Fetch user profile using access token"""
# # # #     try:
# # # #         headers = {"Authorization": f"Bearer {token}"}
# # # #         res = requests.get(f"{API_URL}/auth/profile", headers=headers, timeout=10)
# # # #         if res.status_code == 200:
# # # #             return res.json()
# # # #         else:
# # # #             print("Profile fetch failed:", res.text)
# # # #             return None
# # # #     except Exception as e:
# # # #         print("Profile error:", e)
# # # #         return None
# # # # import requests


# # # # def get_user_profile(token: str):
# # # #     """Fetch logged-in user profile details from backend"""
# # # #     try:
# # # #         headers = {"Authorization": f"Bearer {token}"}
# # # #         res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers, timeout=10)

# # # #         if res.status_code == 200:
# # # #             return res.json()
# # # #         else:
# # # #             print("Profile fetch failed:", res.text)
# # # #             return None
# # # #     except Exception as e:
# # # #         print("Profile error:", e)
# # # #         return None
    
# # # # import requests

# # # # BASE_URL = "http://127.0.0.1:8000"


# # # def get_user_profile(token):
# # #     headers = {"Authorization": f"Bearer {token}"}
# # #     res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers)
# # #     if res.status_code == 200:
# # #         return res.json()
# # #     print("Profile fetch failed:", res.text)
# # #     return None

# # # def update_user_profile(token, data):
# # #     headers = {"Authorization": f"Bearer {token}"}
# # #     res = requests.put(f"{BACKEND_URL}/auth/profile", headers=headers, json=data)
# # #     return res.status_code == 200


# # # def upload_profile_image(token, image_file):
# # #     headers = {"Authorization": f"Bearer {token}"}
# # #     files = {"file": image_file}
# # #     res = requests.post(f"{BACKEND_URL}/auth/profile/image", headers=headers, files=files)
# # #     return res.status_code == 200

# # import requests

# # # ✅ Backend API Base URL (change if deployed)
# # BACKEND_URL = "http://127.0.0.1:8000/api/v1"


# # # ---------------- AUTH ----------------
# # def signup(username: str, email: str, password: str):
# #     """Register a new user"""
# #     try:
# #         res = requests.post(
# #             f"{BACKEND_URL}/auth/signup",
# #             json={"username": username, "email": email, "password": password},
# #             timeout=10
# #         )
# #         return res
# #     except Exception as e:
# #         print("Signup error:", e)
# #         return None


# # def login(email: str, password: str):
# #     """Login existing user and get token"""
# #     try:
# #         res = requests.post(
# #             f"{BACKEND_URL}/auth/login",
# #             data={"username": email, "password": password},
# #             timeout=10
# #         )
# #         return res
# #     except Exception as e:
# #         print("Login error:", e)
# #         return None


# # # ---------------- PREDICT ----------------
# # def predict(image, token: str):
# #     """Send image to model prediction endpoint"""
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         files = {"file": image}
# #         res = requests.post(f"{BACKEND_URL}/predict", headers=headers, files=files, timeout=30)
# #         return res
# #     except Exception as e:
# #         print("Prediction error:", e)
# #         return None


# # # ---------------- PROFILE ----------------
# # def get_user_profile(token: str):
# #     """Fetch user profile"""
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers, timeout=10)

# #         if res.status_code == 200:
# #             return res.json()
# #         else:
# #             print("Profile fetch failed:", res.text)
# #             return None
# #     except Exception as e:
# #         print("Profile error:", e)
# #         return None


# # def update_user_profile(token: str, data: dict):
# #     """Update user profile (e.g., username, email)"""
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         res = requests.put(f"{BACKEND_URL}/auth/profile", headers=headers, json=data, timeout=10)
# #         return res.status_code == 200
# #     except Exception as e:
# #         print("Profile update error:", e)
# #         return False


# # def upload_profile_image(token: str, image_file):
# #     """Upload new profile picture"""
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         files = {"file": image_file}
# #         res = requests.post(f"{BACKEND_URL}/auth/profile/image", headers=headers, files=files, timeout=15)
# #         return res.status_code == 200
# #     except Exception as e:
# #         print("Profile image upload error:", e)
# #         return False



# # import requests

# # # Backend base
# # BACKEND_URL = "http://127.0.0.1:8000/api/v1"

# # # --- AUTH ---
# # def signup(username: str, email: str, password: str):
# #     try:
# #         return requests.post(
# #             f"{BACKEND_URL}/auth/signup",
# #             json={"username": username, "email": email, "password": password},
# #             timeout=10,
# #         )
# #     except Exception as e:
# #         print("Signup error:", e)
# #         return None

# # def login(email: str, password: str):
# #     """
# #     The backend login expects JSON {email, password} (adjust if your backend uses form-data).
# #     Returns the full Response object so front-end can check status / message.
# #     """
# #     try:
# #         return requests.post(
# #             f"{BACKEND_URL}/auth/login",
# #             data={"username": email, "password": password},
# #             timeout=10,
# #         )
# #     except Exception as e:
# #         print("Login error:", e)
# #         return None

# # # --- PROFILE ---
# # def get_user_profile(token: str):
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers, timeout=10)
# #         if res.status_code == 200:
# #             return res.json()
# #         else:
# #             print("Profile fetch failed:", res.text)
# #             return None
# #     except Exception as e:
# #         print("Profile error:", e)
# #         return None

# # def update_user_profile(token: str, data: dict):
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}
# #         res = requests.put(f"{BACKEND_URL}/auth/profile", headers=headers, json=data, timeout=10)
# #         return res
# #     except Exception as e:
# #         print("Profile update error:", e)
# #         return None


# # # --- PREDICT ---

# # # def predict(token: str, image_file):
# # #     try:
# # #         headers = {"Authorization": f"Bearer {token}"}
# # #         files = {"file": (image_file.name, image_file.getvalue(), image_file.type)}

# # #         res = requests.post(
# # #             f"{BACKEND_URL}/predict/",
# # #             files=files,
# # #             headers=headers,
# # #             timeout=30
# # #         )
# # #         return res
# # #     except Exception as e:
# # #         print("Prediction error:", e)
# # #         return None
# # import requests

# # def predict(token: str, image_file):
# #     """
# #     Send image to backend for breed prediction.
# #     Returns: response object or None on error.
# #     """
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"} if token else {}

# #         files = {
# #             "file": (image_file.name, image_file.getvalue(), image_file.type)
# #         }

# #         res = requests.post(
# #             f"{BACKEND_URL}/predict/",
# #             files=files,
# #             headers=headers,
# #             timeout=30
# #         )
# #         return res

# #     except Exception as e:
# #         print("Prediction error:", e)
# #         return None


# # def upload_profile_image(token: str, image_file):
# #     """
# #     Uploads profile image to backend.
# #     Returns: JSON on success else None.
# #     """
# #     try:
# #         headers = {"Authorization": f"Bearer {token}"}

# #         files = {
# #             "file": (
# #                 image_file.name,
# #                 image_file.getvalue(),
# #                 image_file.type
# #             )
# #         }

# #         res = requests.post(
# #             f"{BACKEND_URL}/auth/profile/image",
# #             headers=headers,
# #             files=files,
# #             timeout=15
# #         )

# #         if res.status_code in (200, 201):
# #             return res.json()
# #         else:
# #             print("Profile upload failed:", res.text)
# #             return None

# #     except Exception as e:
# #         print("Profile upload error:", e)
# #         return None


# import requests

# BACKEND_URL = "http://127.0.0.1:8000/api/v1"

# # --- AUTH ---
# def signup(username: str, email: str, password: str):
#     try:
#         return requests.post(
#             f"{BACKEND_URL}/auth/signup",
#             json={"username": username, "email": email, "password": password},
#             timeout=10,
#         )
#     except Exception as e:
#         print("Signup error:", e)
#         return None

# def login(email: str, password: str):
#     try:
#         return requests.post(
#             f"{BACKEND_URL}/auth/login",
#             data={"username": email, "password": password},
#             timeout=10,
#         )
#     except Exception as e:
#         print("Login error:", e)
#         return None

# # --- PROFILE ---
# def get_user_profile(token: str):
#     try:
#         headers = {"Authorization": f"Bearer {token}"}
#         res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers, timeout=10)
#         if res.status_code == 200:
#             return res.json()
#         return None
#     except Exception as e:
#         print("Profile error:", e)
#         return None

# def update_user_profile(token: str, data: dict):
#     try:
#         headers = {"Authorization": f"Bearer {token}"}
#         res = requests.put(f"{BACKEND_URL}/auth/profile", headers=headers, json=data, timeout=10)
#         return res
#     except Exception as e:
#         print("Profile update error:", e)
#         return None

# # --- PREDICTION ---
# def predict(token: str, image_file):
#     try:
#         headers = {"Authorization": f"Bearer {token}"} if token else {}
#         files = {"file": (image_file.name, image_file.getvalue(), image_file.type)}
#         res = requests.post(f"{BACKEND_URL}/predict/", files=files, headers=headers, timeout=30)
#         return res
#     except Exception as e:
#         print("Prediction error:", e)
#         return None

# # --- UPLOAD PROFILE IMAGE ---
# def upload_profile_image(token: str, image_file):
#     try:
#         headers = {"Authorization": f"Bearer {token}"}
#         files = {"file": (image_file.name, image_file.getvalue(), image_file.type)}
#         res = requests.post(f"{BACKEND_URL}/auth/profile/image", headers=headers, files=files, timeout=15)
#         if res.status_code in (200, 201):
#             return res.json()
#         return None
#     except Exception as e:
#         print("Profile upload error:", e)
#         return None

# utils/api_helper.py
import requests

BACKEND_URL = "http://127.0.0.1:8000/api/v1"

# --- AUTH ---
def signup(username: str, email: str, password: str):
    try:
        return requests.post(
            f"{BACKEND_URL}/auth/signup",
            json={"username": username, "email": email, "password": password},
            timeout=10,
        )
    except Exception as e:
        print("Signup error:", e)
        return None

def login(email: str, password: str):
    try:
        return requests.post(
            f"{BACKEND_URL}/auth/login",
            data={"username": email, "password": password},
            timeout=10,
        )
    except Exception as e:
        print("Login error:", e)
        return None

# --- FORGOT / RESET PASSWORD ---
def forgot_password(email: str):
    """
    Tries to request a password reset for `email`.
    Backend should expose something like POST /auth/forgot-password with JSON {"email": "..."}
    """
    try:
        return requests.post(f"{BACKEND_URL}/auth/forgot-password", json={"email": email}, timeout=10)
    except Exception as e:
        print("Forgot password error:", e)
        return None

def reset_password(token: str, new_password: str):
    """
    If your backend supports a reset token flow, this function can be used by a reset page.
    POST /auth/reset-password with {"token": "...", "password": "..."}
    """
    try:
        return requests.post(f"{BACKEND_URL}/auth/reset-password", json={"token": token, "password": new_password}, timeout=10)
    except Exception as e:
        print("Reset password error:", e)
        return None

# --- PROFILE ---
def get_user_profile(token: str):
    try:
        headers = {"Authorization": f"Bearer {token}"}
        res = requests.get(f"{BACKEND_URL}/auth/profile", headers=headers, timeout=10)
        if res.status_code == 200:
            return res.json()
        return None
    except Exception as e:
        print("Profile error:", e)
        return None

def update_user_profile(token: str, data: dict):
    try:
        headers = {"Authorization": f"Bearer {token}"}
        res = requests.put(f"{BACKEND_URL}/auth/profile", headers=headers, json=data, timeout=10)
        return res
    except Exception as e:
        print("Profile update error:", e)
        return None

# --- PREDICTION ---
def predict(token: str, image_file):
    try:
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        files = {"file": (image_file.name, image_file.getvalue(), image_file.type)}
        res = requests.post(f"{BACKEND_URL}/predict/", files=files, headers=headers, timeout=30)
        return res
    except Exception as e:
        print("Prediction error:", e)
        return None

# --- UPLOAD PROFILE IMAGE ---
def upload_profile_image(token: str, image_file):
    try:
        headers = {"Authorization": f"Bearer {token}"}
        files = {"file": (image_file.name, image_file.getvalue(), image_file.type)}
        res = requests.post(f"{BACKEND_URL}/auth/profile/image", headers=headers, files=files, timeout=15)
        if res.status_code in (200, 201):
            return res.json()
        return None
    except Exception as e:
        print("Profile upload error:", e)
        return None
